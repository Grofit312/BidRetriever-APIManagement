    ...
    public class CompanyManagementController : Controller
    {
        ...
        public IActionResult UpdateCompany(Company request)
        {
            try
            {
                ...
                using (var cmd = _dbHelper.SpawnCommand())
                {
                    cmd.CommandText = "UPDATE customer_contacts SET "
                        + "company_name = COALESCE(@company_name, company_name), "
                        + "company_office_id = COALESCE(@company_office_id, company_office_id) "
                        + $"WHERE company_id = @company_id";

                    cmd.Parameters.AddWithValue("@company_name", (object)request.company_name ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@company_office_id", (object)request.customer_office_id ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@company_id", request.company_id);
