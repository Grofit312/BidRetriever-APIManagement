            ...
            try
            {
                ...
                using (var cmd = _dbHelper.SpawnCommand())
                {
                    ...
                    cmd.CommandText = "SELECT "
                      ...
                      + "customer_companies.company_name AS source_company_name, source_system_types.source_type_name AS project_source_sys_name, "
                      + "projects.project_id AS secondary_project_id, projects.source_company_id as source_company_id, "
                      + "customer_contacts.contact_id as source_contact_id, projects.project_stage "
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            result.Add(new Dictionary<string, object>
                            {
                                ...
                                { "project_stage", _dbHelper.SafeGetString(reader, 16) },
                            });
                        }
                    }
                }
