    ...
    public IActionResult Post(DLProject project, bool isInternalRequest = false)
    {
      try
      {
        ...
        using (var cmd = _dbHelper.SpawnCommand())
        {
          ...
          {
            var customerSetting = settingsList.Find(setting => setting["setting_id"] == "plan_file_naming");

            if (customerSetting == null)
            {
              customerSetting = settingsList[0];
            }

            destinationAccessToken = customerSetting["destination_access_token"];
            destinationUrl = customerSetting["destination_url"];
            destinationRootPath = customerSetting["destination_root_path"];
            ...
            planFileNaming = customerSetting["setting_id"] == "plan_file_naming" ? customerSetting["setting_value"] : "";
          }
          ...
          if (planFileNaming != string.Empty)
          {
            new ProjectSettingManagementController().Post(new ProjectSetting
            {
              project_id = projectId,
              setting_value = planFileNaming,
      ...
      try
      {
        ...
        using (var cmd = _dbHelper.SpawnCommand())
        {
          ...
          cmd.CommandText = "SELECT projects.create_datetime, users.customer_id, customers.customer_name, "
              ...
              + "projects.project_displayname, ps.project_submission_id as original_submission_id, projects.source_project_id "
              + "FROM projects "
              + "LEFT JOIN users ON users.user_id=projects.project_admin_user_id "
